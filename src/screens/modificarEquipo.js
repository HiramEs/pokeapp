...
const modificarEquipo = ({ route  }) => {
    ...
    useEffect(async () => {
        const response = await firestore().collection("Equipos").doc(route.params.ID).get();
        console.log(response.data());
        setNombreEquipo(response.data().NombreEquipo);
    ...
    const GuardarEquipo = async () => {
        if (pokemones.length > 2 && pokemones.length <= 6 && NombreEquipo != '') {
            firestore().collection("Equipos").doc(ID).update({
                NombreEquipo,
                pokemones,
            });
            navigator.goBack();
        ...
        <SafeAreaView style={styles.mainContainer} >
            <View style={styles.mainContainer}>
                <View style={styles.nombreContainer}>
                    ...
                    <TextInput style={styles.equipoInput} onChangeText={setNombreEquipo} value={NombreEquipo} />
                    {pokemones.length == 0 ? (
